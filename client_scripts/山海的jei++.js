...
JEIEvents.addItems(function(event) {
    ...
    try {
        ...
        if (dl && dl.length > 0) {
            ...
            event.add(Item.of('gt_shanhai:super_disk_array', DShanhaiNBTAPI.buildSDAFromList(di, '无限染料元件包pro', ['§7包含所有染料物品的无限元件包','§7染料种类: §e' + uniqueDyes.length + '§7 种','§7每个染料存储在无限元件包中','§8山海私货 v2.2'], [])));
        }
    } catch(e) { console.error('[山海JEI] 染料包失败: ' + e); }
    console.log('[山海JEI] 已注册基础超级磁盘阵列变体与染料包；礼包由 Java JEI 插件提供');
});
...
'',
"Ω-|████████████████",
'Ω-普通提示：{electric}【重力维护仓是什么？】',
'|重力是配方显式需求，例如：“需要强重力环境”',
'|那么你就需要重力维护仓了',
'|维护仓可调整：100即为强重力，0即为无重力'
],[
'',
'Ω-|███████████████',
'按住 ALT 查看神私普通提示'
