...
let myRecipes = [
  ...
  { id: 'coin_forge_magmatter_coin_source', type: 'coin_forge',notConsumable: ["gtceu:credit_casting_mold"], itemInputs: ["9x dishanhai:chaos_coin"], inputFluids: ['gtceu:magmatter 36000'], itemOutputs: ["16x dishanhai:magmatter_coin"], EUt: max},
  { id: 'coin_forge_eternity_coin_source', type: 'coin_forge',notConsumable: ["gtceu:credit_casting_mold","kubejs:eternity_catalyst"], itemInputs: ["9x dishanhai:magmatter_coin"], inputFluids: ['gtceu:eternity 36000'], itemOutputs: ["16x dishanhai:eternity_coin"], EUt: max},
...
if (Platform.isLoaded('ae2_overclocked')){
    ...
    //&& ========== 通用无限流体元件包配方生成函数 ==========
    function generateInfinityFluidPackRecipe(config) {
        var recipeType = 'assembler';
        var recipeId = config.recipeId;
        var filterRegex = config.filterRegex;
        var itemName = config.itemName;
        var desc1 = config.desc1;
        var desc2 = config.desc2;

        try {
            info('🎨 开始生成' + itemName + '配方...');
            ...
            if (!fluidList || fluidList.length === 0) {
                throw new Error('未找到匹配的流体');
            }
            info('🎨 找到 ' + fluidList.length + ' 种流体');

            gtr.assembler(recipeId)
                .circuit(1)
                .itemInputs('minecraft:dandelion')
                .itemOutputs(DShanhaiSDA.create(itemName)
                    .infinityOutputs(fluidList)
                    .lore([
                        '§7' + desc1,
                        '§7' + desc2 + fluidList.length + '§7 种',
                        '§7每个条目存储在无限元件包中',
                        '§8山海私货 v2.7'
                    ])
                    .build())
            ...
            try { DShanhaiRecipeEngine.recordRecipe(recipeType, true, recipeId, ''); } catch(ignored) {}
            info('✅ ' + itemName + '配方已生成');
        } catch(err) {
            error('❌ ' + itemName + '配方生成失败: ' + err.message);
            try { DShanhaiRecipeEngine.recordRecipe(recipeType, false, recipeId, err.message); } catch(ignored) {}
        }
    }

    //&& ========== 无限gtceu流体元件包配方（可选）==========
    /*
    generateInfinityFluidPackRecipe({
        recipeId: 'dishanhai:infinity_gtceu_fluid_pack',
        filterRegex: /^gtceu:.+/,
        itemName: '超级磁盘阵列',
        desc1: '包含所有gtceu流体的无限元件包',
        desc2: '流体种类: §e'
    });
    */
    ...
    /*
    generateInfinityFluidPackRecipe({
        recipeId: 'dishanhai:infinity_plasma_fluid_pack',
        filterRegex: /plasma$/,
        itemName: '无限油元件包',
        desc1: '包含所有油的无限元件包',
        desc2: '物品种类: §e'
    ...
    */
    ...
    //&& ========== 原始终焉引擎建材 SASDA ==========
    var primordialOmegaEngineMaterialsList = [
        '11008x gtceu:industrial_steam_casing',
        '6550x gtceu:bronze_machine_casing',
        '1566x gtceu:bronze_pipe_casing',
        '357x gtceu:bronze_brick_casing',
        '249x gtceu:firebricks',
        '63x gtceu:steam_machine_casing',
        '36x gtceu:coke_oven_bricks',
        '16x kubejs:steam_assembly_block',
        '1x gt_shanhai:primordial_omega_engine'
    ];


    //&& ========== 原始终焉引擎建材配方 ==========
    var primordialOmegaMaterialsRecipeType = 'assembler';
    var primordialOmegaMaterialsRecipeId = 'dishanhai:primordial_omega_engine_materials';

    try {
        info('🔧 开始生成原始终焉引擎建材配方...');
        var ok = safeAddRecipe(primordialOmegaMaterialsRecipeType, primordialOmegaMaterialsRecipeId, function() {
            gtr.assembler(primordialOmegaMaterialsRecipeId)
                .itemInputs('dishanhai:cshx')
                .itemOutputs(DShanhaiPackRegistry.create('primordialOmegaMaterials', primordialOmegaEngineMaterialsList, '原始终焉引擎建材', [
        '§7原始终焉引擎的核心建材集合',
        '§7物品种类: §e%count%§7 种',
        '§7包含引擎全部构成材料',
        '§8山海私货 v2.7.3'
        ...
        .build())
                .circuit(8)
                .duration(400)
                .EUt(65565 * GTValues.VA[14]);
        });
        if (ok) {
            try { DShanhaiRecipeEngine.recordRecipe(primordialOmegaMaterialsRecipeType, true, primordialOmegaMaterialsRecipeId, ''); } catch(ignored) {}
            info('✅ 原始终焉引擎建材配方已生成');
        }
    } catch(err) {
        error('❌ 原始终焉引擎建材配方生成失败: ' + err.message);
        try { DShanhaiRecipeEngine.recordRecipe(primordialOmegaMaterialsRecipeType, false, primordialOmegaMaterialsRecipeId, err.message); } catch(ignored) {}
    }
