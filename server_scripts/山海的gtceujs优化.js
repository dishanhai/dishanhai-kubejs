// ===== gtceu.js 的山海接管副本：内容与原 gtceu.js 逐行一致（未做任何数据表重写），=====
// ===== gtceu.js 本体已改名 .bak 禁用。这里不接缓存命中跳过——文件里混了
// ===== gtr.xxx()（GTRecipe，能被 GTRecipeSerializer.CODEC 导出）和 event.shapeless/shaped()
// ===== （原版 ShapedRecipe/ShapelessRecipe，导不出来）两种配方类型，还有 kubejs: 等非 gtceu:
// ===== 命名空间的配方，缓存兜底导出做不到不漏，所以每次都老老实实完整跑一遍。=====
//priority: 96
